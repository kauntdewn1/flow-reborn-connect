...
import IconMissions from '/public/MISSOES.png';
import IconAlert from '/public/ALERTA.png';
import RewardBox from '/public/reward.png';
import GlitchButtonBg from '/public/button glitch.png';
  ...
  return (
    <div className="bg-black/70 border border-cyan-400/20 rounded-xl p-4 mt-6">
      <div className="flex items-center gap-2 mb-4">
        <img src={IconMissions} alt="Missões" className="w-6 h-6" />
        ...
        {availableMissions.map((mission) => (
          <div key={mission.id} className="bg-[#131313] border border-cyan-400/10 rounded-lg p-3 relative">
            <div className="absolute top-2 right-2">
              <img src={IconAlert} alt="Alerta" className="w-4 h-4 opacity-60" />
            </div>
            ...
            <div className="flex items-center justify-between text-xs text-gray-300 mt-2">
              <span>XP: <span className="text-accent-glitch font-bold">{mission.rewardXp}</span></span>
              <span>FRAG: <span className="text-accent-glitch font-bold">{mission.rewardFragments}</span></span>
              <img src={RewardBox} alt="Recompensa" className="w-5 h-5" />
            </div>
            <button
              ...
              style={{ backgroundImage: `url(${GlitchButtonBg})`, backgroundSize: 'cover' }}
            >
              ACEITAR MISSÃO
            </button>
          </div>
